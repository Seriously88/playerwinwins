...
YELLOW_RACKET = (220, 180, 50)    # Yellow racket color to remove
...
output_dir = "assests/player_animation_improved"
os.makedirs(output_dir, exist_ok=True)

# Function to remove background and racket from a surface
def remove_background_and_racket(surface):
    ...
    for y in range(surface.get_height()):
        for x in range(surface.get_width()):
            ...
            # Check if the color is close to the blue background or yellow racket
            if ((abs(color[0] - BLUE_BACKGROUND[0]) < COLOR_THRESHOLD and
                abs(color[1] - BLUE_BACKGROUND[1]) < COLOR_THRESHOLD and
                abs(color[2] - BLUE_BACKGROUND[2]) < COLOR_THRESHOLD) or
                (abs(color[0] - YELLOW_RACKET[0]) < COLOR_THRESHOLD and
                abs(color[1] - YELLOW_RACKET[1]) < COLOR_THRESHOLD and
                abs(color[2] - YELLOW_RACKET[2]) < COLOR_THRESHOLD)):
...
for row in range(rows):
    for col in range(cols):
        ...
        # Remove blue background and yellow racket
        sprite_transparent = remove_background_and_racket(sprite)
